<template>
  <section>
    <nav class="relative px-6 py-6 flex justify-between items-center" style="background: var(--primary)">
      <span class="text-white text-2xl text-white font-bold leading-none">

        <div class="hidden absolute top-1/2 left-1/2 transform -translate-y-1/2 -translate-x-1/2 sm:flex sm:mx-auto sm:items-center sm:space-x-6" >
          <div v-for="item in menu.items" :key="item.path">
            <nuxt-link class="text-lg text-white hover:text-white" :to="item.path">
              {{ item.title }}
            </nuxt-link>
          </div>
        </div>
          ...
          <div class="p-6">
            ...
            <div class="mt-10">
              <div v-for="item in menu.items" :key="item.path" class="mt-3">
                <nuxt-link class="text-sm text-white" :to="item.path" @click.native="$refs.menu.close" @click="$refs.menu.close()">
                  {{ item.title }}
                </nuxt-link>
              </div>
            </div>
        ...
        <Logo class="pl-6 sm:pl-0 sm:relative absolute" />

      </span>

      <!-- Social icons-->
      <social-icons class="hidden sm:flex" />
    </nav>

    <!--       header only if in the page main-->
<!--    <div class="relative pt-12 border-b-4 border-primary"  style="background: var(&#45;&#45;primary)">-->
